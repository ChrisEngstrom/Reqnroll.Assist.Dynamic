...
public static class DynamicTableHelpers
{
    ...
    private static object CreateTypedValue(string valueFromTable, bool doTypeConversion = true)
    {
        ...
        {
            if (Decimal.TryParse(valueFromTable, out decimal d) && d.Equals((decimal)db))
            {
                return db;
            }
        ...
        if (Boolean.TryParse(valueFromTable, out bool b))
        {
            return b;
        }

        if (DateTime.TryParse(valueFromTable, out DateTime dt))
        {
            return dt;
        }
